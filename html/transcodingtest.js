...
var janus = null;
var transcode = null;
var opaqueId = "transcodetest-"+Janus.randomString(12);
...
var playing = false;
var transcodingId = null;
var selectedTranscoding = null;
var selectedTranscodingInfo = null;
			...
			janus = new Janus(
				{
					...
					success: function() {
						...
						janus.attach(
							{
								plugin: "janus.plugin.transcode",
								opaqueId: opaqueId,
								success: function(pluginHandle) {
									$('#details').remove();
									transcode = pluginHandle;
									Janus.log("Plugin attached! (" + transcode.getPlugin() + ", id=" + transcode.getId() + ")");
									// Prepare the name prompt
									$('#transcode').removeClass('hide').show();
									...
									if(result !== null && result !== undefined) {
										if(result["status"] !== undefined && result["status"] !== null) {
											...
											if(event === 'preparing' || event === 'refreshing') {
												Janus.log("Preparing the transcoding playout");
												transcode.createAnswer(
													{
														jsep: jsep,
														...
														success: function(jsep) {
															...
															transcode.send({"message": body, "jsep": jsep});
														},
														error: function(error) {
											...
											} else if(event === 'transcoding') {
												// Got an ANSWER to our transcoding OFFER
												if(jsep !== null && jsep !== undefined)
													transcode.handleRemoteJsep({jsep: jsep});
												var id = result["id"];
												if(id !== null && id !== undefined) {
													Janus.log("The ID of the current transcoding is " + id);
													transcodingId = id;
												}
												...
												var id = result["id"];
												if(transcodingId !== null && transcodingId !== undefined) {
													if(transcodingId !== id) {
														Janus.warn("Not a stop to our transcoding?");
														return;
													}
													bootbox.alert("Transcoding completed! Check the list of transcodings to replay it.");
												}
												if(selectedTranscoding !== null && selectedTranscoding !== undefined) {
													if(selectedTranscoding !== id) {
												...
												playing = false;
												transcode.hangup();
												$('#transcode').removeAttr('disabled').click(startTranscoding);
								...
								onlocalstream: function(stream) {
									...
									if(transcode.webrtcStuff.pc.iceConnectionState !== "completed" &&
											transcode.webrtcStuff.pc.iceConnectionState !== "connected") {
								...
								onremotestream: function(stream) {
									...
									Janus.debug(stream);
									if($('#thevideo').length === 0) {
										$('#videotitle').html(selectedTranscodingInfo);
										$('#stop').unbind('click').click(stop);
										$('#video').removeClass('hide').show();
...
function updateRecsList() {
	...
	transcode.send({"message": body, success: function(result) {
		setTimeout(function() {
			$('#list').click(updateRecsList);
			$('#update-list').removeClass('fa-spin');
		}, 500);
		if(result === null || result === undefined) {
			bootbox.alert("Got no response to our query for available transcodings");
			return;
		}
		if(result["list"] !== undefined && result["list"] !== null) {
			...
			Janus.debug("Got a list of available transcodings:");
			Janus.debug(list);
			for(var mp in list) {
			...
			$('#recslist a').unbind('click').click(function() {
				selectedTranscoding = $(this).attr("id");
				selectedTranscodingInfo = $(this).text();
...
function startTranscoding() {
	...
	transcoding = true;
	playing = false;
	bootbox.prompt("Insert a name for the transcoding (e.g., John Smith says hello)", function(result) {
		if(result === null || result === undefined) {
			transcoding = false;
			return;
		}
		myname = result;
		$('#transcode').unbind('click').attr('disabled', true);
		...
		// before, after, during transcoding
		transcode.send({
			'message': {
				'request': 'configure',
		...
		transcode.createOffer(
			{
				...
				success: function(jsep) {
					Janus.debug("Got SDP!");
					Janus.debug(jsep);
					var body = { "request": "transcode", "name": myname };
					transcode.send({"message": body, "jsep": jsep});
				},
				error: function(error) {
					Janus.error("WebRTC error...", error);
...
function startPlayout() {
	...
	transcoding = false;
	playing = true;
	if(selectedTranscoding === undefined || selectedTranscoding === null) {
		playing = false;
		return;
	}
	$('#transcode').unbind('click').attr('disabled', true);
	...
	$('#recset').attr('disabled', true);
	$('#recslist').attr('disabled', true);
	var play = { "request": "play", "id": parseInt(selectedTranscoding) };
	transcode.send({"message": play});
}

function stop() {
	// Stop a transcoding/playout
	$('#stop').unbind('click');
	var stop = { "request": "stop" };
	transcode.send({"message": stop});
	transcode.hangup();
}
