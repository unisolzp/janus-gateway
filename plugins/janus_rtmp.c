...
int janus_rtmp_init(janus_callbacks *callback, const char *onfig_path);
void janus_rtmp_destroy(void);
int janus_rtmp_get_api_compatibility(void);
int janus_rtmp_get_version(void);
const char *janus_rtmp_get_version_string(void);
const char *janus_rtmp_get_description(void);
const char *janus_rtmp_get_name(void);
const char *janus_rtmp_get_author(void);
const char *janus_rtmp_get_package(void);
void janus_rtmp_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *janus_rtmp_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep);
json_t *janus_rtmp_handle_admin_message(json_t *message);
void janus_rtmp_setup_media(janus_plugin_session *handle);
void janus_rtmp_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_rtmp_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_rtmp_slow_link(janus_plugin_session *handle, int uplink, int video);
void janus_rtmp_hangup_media(janus_plugin_session *handle);
void janus_rtmp_destroy_session(janus_plugin_session *handle, int *error);
json_t *janus_rtmp_query_session(janus_plugin_session *handle);

/* Plugin setup */
static janus_plugin janus_rtmp_plugin =
	JANUS_PLUGIN_INIT (
		.init = janus_rtmp_init,
		.destroy = janus_rtmp_destroy,

		.get_api_compatibility = janus_rtmp_get_api_compatibility,
		.get_version = janus_rtmp_get_version,
		.get_version_string = janus_rtmp_get_version_string,
		.get_description = janus_rtmp_get_description,
		.get_name = janus_rtmp_get_name,
		.get_author = janus_rtmp_get_author,
		.get_package = janus_rtmp_get_package,

		.create_session = janus_rtmp_create_session,
		.handle_message = janus_rtmp_handle_message,
		.handle_admin_message = janus_rtmp_handle_admin_message,
		.setup_media = janus_rtmp_setup_media,
		.incoming_rtp = janus_rtmp_incoming_rtp,
		.incoming_rtcp = janus_rtmp_incoming_rtcp,
		.slow_link = janus_rtmp_slow_link,
		.hangup_media = janus_rtmp_hangup_media,
		.destroy_session = janus_rtmp_destroy_session,
		.query_session = janus_rtmp_query_session,
	);
...
static void janus_rtmp_hangup_media_internal(janus_plugin_session *handle);

typedef struct janus_rtmp_message {
...
static janus_rtmp_message exit_message;

typedef struct janus_rtmp_rtp_header_extension {
	uint16_t type;
	uint16_t length;
} janus_rtmp_rtp_header_extension;

typedef struct janus_rtmp_frame_packet {
	uint16_t seq;	/* RTP Sequence number */
	uint64_t ts;	/* RTP Timestamp */
	int len;		/* Length of the data */
	long offset;	/* Offset of the data in the file */
	struct janus_rtmp_frame_packet *next;
	struct janus_rtmp_frame_packet *prev;
} janus_rtmp_frame_packet;
janus_rtmp_frame_packet *janus_rtmp_get_frames(const char *dir, const char *filename);

typedef struct janus_rtmp_recording {
...
typedef struct janus_rtmp_session {
	...
	janus_rtmp_recording *recording;
	janus_recorder *arc;	/* Audio recorder */
	janus_recorder *vrc;	/* Video recorder */
	janus_rtmp_pub *pub;	/* live pub */
	janus_mutex rec_mutex;	/* Mutex to protect the recorders from race conditions */
	janus_rtmp_frame_packet *aframes;	/* Audio frames (for playout) */
	janus_rtmp_frame_packet *vframes;	/* Video frames (for playout) */
...
static void janus_rtmp_session_destroy(janus_rtmp_session *session) {
	if(session && g_atomic_int_compare_and_exchange(&session->destroyed, 0, 1))
		janus_refcount_decrease(&session->ref);
}

static void janus_rtmp_session_free(const janus_refcount *session_ref) {
	janus_rtmp_session *session = janus_refcount_containerof(session_ref, janus_rtmp_session, ref);
...
static void janus_rtmp_recording_destroy(janus_rtmp_recording *recording) {
	if(recording && g_atomic_int_compare_and_exchange(&recording->destroyed, 0, 1))
		janus_refcount_decrease(&recording->ref);
}

static void janus_rtmp_recording_free(const janus_refcount *recording_ref) {
	janus_rtmp_recording *recording = janus_refcount_containerof(recording_ref, janus_rtmp_recording, ref);
...
void janus_rtmp_update_recordings_list(void);
static void *janus_rtmp_playout_thread(void *data);

/* Helper to send RTCP feedback back to recorders, if needed */
void janus_rtmp_send_rtcp_feedback(janus_plugin_session *handle, int video, char *buf, int len);
...
static const char *janus_rtmp_parse_codec(const char *dir, const char *filename) {
...
static int janus_rtmp_generate_offer(janus_rtmp_recording *rec) {
	if(rec == NULL)
		return -1;
...
static void janus_rtmp_message_free(janus_rtmp_message *msg) {
	if(!msg || msg == &exit_message)
		return;

	if(msg->handle && msg->handle->plugin_handle) {
		janus_rtmp_session *session = (janus_rtmp_session *)msg->handle->plugin_handle;
		janus_refcount_decrease(&session->ref);
	}
	msg->handle = NULL;
...
/* Error codes */
#define JANUS_RTMP_ERROR_NO_MESSAGE			411
#define JANUS_RTMP_ERROR_INVALID_JSON			412
#define JANUS_RTMP_ERROR_INVALID_REQUEST		413
#define JANUS_RTMP_ERROR_INVALID_ELEMENT		414
#define JANUS_RTMP_ERROR_MISSING_ELEMENT		415
#define JANUS_RTMP_ERROR_NOT_FOUND			416
#define JANUS_RTMP_ERROR_INVALID_RECORDING	417
#define JANUS_RTMP_ERROR_INVALID_STATE		418
#define JANUS_RTMP_ERROR_INVALID_SDP			419
#define JANUS_RTMP_ERROR_RECORDING_EXISTS		420
#define JANUS_RTMP_ERROR_UNKNOWN_ERROR		499


/* Plugin implementation */
int janus_rtmp_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.jcfg", config_path, JANUS_RTMP_PACKAGE);
	JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	janus_config *config = janus_config_parse(filename);
	if(config == NULL) {
		JANUS_LOG(LOG_WARN, "Couldn't find .jcfg configuration file (%s), trying .cfg\n", JANUS_RTMP_PACKAGE);
		g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_RTMP_PACKAGE);
		JANUS_LOG(LOG_VERB, "Configuration file: %s\n", filename);
	...
	recordings = g_hash_table_new_full(g_int64_hash, g_int64_equal, (GDestroyNotify)g_free, (GDestroyNotify)janus_rtmp_recording_destroy);
	janus_rtmp_update_recordings_list();

	sessions = g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify)janus_rtmp_session_destroy);
	messages = g_async_queue_new_full((GDestroyNotify) janus_rtmp_message_free);
...
int janus_rtmp_get_version(void) {
	return JANUS_RTMP_VERSION;
}

const char *janus_rtmp_get_version_string(void) {
	return JANUS_RTMP_VERSION_STRING;
}

const char *janus_rtmp_get_description(void) {
	return JANUS_RTMP_DESCRIPTION;
}

const char *janus_rtmp_get_name(void) {
	return JANUS_RTMP_NAME;
}

const char *janus_rtmp_get_author(void) {
	return JANUS_RTMP_AUTHOR;
}

const char *janus_rtmp_get_package(void) {
	return JANUS_RTMP_PACKAGE;
}

static janus_rtmp_session *janus_rtmp_lookup_session(janus_plugin_session *handle) {
	janus_rtmp_session *session = NULL;
	if (g_hash_table_contains(sessions, handle)) {
		session = (janus_rtmp_session *)handle->plugin_handle;
	}
	return session;
}
...
void janus_rtmp_create_session(janus_plugin_session *handle, int *error) {
	...
	janus_rtmp_session *session = g_malloc0(sizeof(janus_rtmp_session));
...
struct janus_plugin_result *janus_rtmp_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	...
	if(!session) {
		janus_mutex_unlock(&sessions_mutex);
		JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		error_code = JANUS_RTMP_ERROR_UNKNOWN_ERROR;
	...
	if(message == NULL) {
		...
		error_code = JANUS_RTMP_ERROR_NO_MESSAGE;
		g_snprintf(error_cause, 512, "%s", "No message??");
		goto plugin_response;
	}
	if(!json_is_object(root)) {
		JANUS_LOG(LOG_ERR, "JSON error: not an object\n");
		error_code = JANUS_RTMP_ERROR_INVALID_JSON;
	...
	JANUS_VALIDATE_JSON_OBJECT(root, request_parameters,
		...
		JANUS_RTMP_ERROR_MISSING_ELEMENT, JANUS_RTMP_ERROR_INVALID_ELEMENT);
	if(error_code != 0)
		goto plugin_response;
	...
	if(!strcasecmp(request_text, "update")) {
		...
		json_object_set_new(response, "rtmp", json_string("ok"));
		goto plugin_response;
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			janus_rtmp_recording *rec = value;
		...
		/* Send info back */
		response = json_object();
		json_object_set_new(response, "rtmp", json_string("list"));
		...
		response = json_object();
		json_object_set_new(response, "rtmp", json_string("configure"));
		...
		janus_rtmp_message *msg = g_malloc(sizeof(janus_rtmp_message));
		msg->handle = handle;
		...
		error_code = JANUS_RTMP_ERROR_INVALID_REQUEST;
...
plugin_response:
		{
			...
			if(error_code != 0) {
				...
				json_object_set_new(event, "rtmp", json_string("event"));
...
admin_response:
		{
			if(!response) {
				/* Prepare JSON error event */
				response = json_object();
				json_object_set_new(response, "rtmp", json_string("event"));
...
void janus_rtmp_setup_media(janus_plugin_session *handle) {
	...
	if(!session->recorder) {
		GError *error = NULL;
		janus_refcount_increase(&session->ref);
		g_thread_try_new("rtmp playout thread", &janus_rtmp_playout_thread, session, &error);
		if(error != NULL) {
			janus_refcount_decrease(&session->ref);
...
void janus_rtmp_send_rtcp_feedback(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	janus_rtmp_session *session = (janus_rtmp_session *)handle->plugin_handle;
	char rtcpbuf[24];
...
void janus_rtmp_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(video && (session->ssrc[0] != 0 || session->rid[0] != NULL)) {
		...
		janus_rtmp_pub_save_frame(session->pub, buf, len, TRUE, 1);
		...
		janus_rtmp_pub_save_frame(session->pub, buf, len, video, 1);
	}

	janus_rtmp_send_rtcp_feedback(handle, video, buf, len);
}
...
void janus_rtmp_slow_link(janus_plugin_session *handle, int uplink, int video) {
	...
	gateway->push_event(session->handle, &janus_rtmp_plugin, NULL, event, NULL);
	json_decref(event);
	janus_refcount_decrease(&session->ref);
}
...
static void janus_rtmp_hangup_media_internal(janus_plugin_session *handle) {
	...
	int ret = gateway->push_event(handle, &janus_rtmp_plugin, NULL, event, NULL);
	...
	if(session->pub) {
		janus_rtmp_pub *pub = session->pub;
		session->pub = NULL;
		janus_rtmp_pub_close(pub);
		JANUS_LOG(LOG_INFO, "Closed rtmp living %s\n", pub->url ? pub->url : "??");
		janus_rtmp_pub_destroy(pub);
	}
	...
	if(session->recorder) {
		if(session->recording) {
			...
			if(file == NULL) {
				...
				/* Generate the offer */
				if(janus_rtmp_generate_offer(session->recording) < 0) {
...
static void *janus_rtmp_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		janus_mutex_lock(&sessions_mutex);
		janus_rtmp_session *session = janus_rtmp_lookup_session(msg->handle);
		if(!session) {
			janus_mutex_unlock(&sessions_mutex);
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
			janus_rtmp_message_free(msg);
		...
		if(!strcasecmp(request_text, "record")) {
			if(!msg_sdp || !msg_sdp_type || strcasecmp(msg_sdp_type, "offer")) {
				...
				error_code = JANUS_RTMP_ERROR_MISSING_ELEMENT;
			...
			if(offer == NULL) {
				...
				error_code = JANUS_RTMP_ERROR_INVALID_SDP;
			...
			if(sdp_update) {
				...
				if(!session->recorder || !session->recording) {
					...
					error_code = JANUS_RTMP_ERROR_INVALID_STATE;
			...
			if(rec_id) {
				...
				if(id > 0) {
					...
					if(g_hash_table_lookup(recordings, &id) != NULL) {
						...
						error_code = JANUS_RTMP_ERROR_RECORDING_EXISTS;
			...
			rec = g_malloc0(sizeof(janus_rtmp_recording));
			rec->id = id;
			...
			janus_refcount_init(&rec->ref, janus_rtmp_recording_free);
			...
			session->pub = janus_rtmp_pub_create(rtmpurl, janus_audiocodec_name(rec->acodec), janus_audiocodec_name(rec->vcodec));
...
recdone:
			...
			if(!sdp_update && notify_events && gateway->events_is_enabled()) {
				...
				gateway->notify_event(&janus_rtmp_plugin, session->handle, info);
			}
		} else if(!strcasecmp(request_text, "play")) {
			if(msg_sdp) {
				JANUS_LOG(LOG_ERR, "A play request can't contain an SDP\n");
				error_code = JANUS_RTMP_ERROR_INVALID_ELEMENT;
			...
			if(rec == NULL || rec->offer == NULL || g_atomic_int_get(&rec->destroyed)) {
				...
				error_code = JANUS_RTMP_ERROR_NOT_FOUND;
				g_snprintf(error_cause, 512, "No such recording");
				goto error;
			}
			/* Access the frames */
			if(rec->arc_file) {
				session->aframes = janus_rtmp_get_frames(recordings_path, rec->arc_file);
			...
			if(rec->vrc_file) {
				session->vframes = janus_rtmp_get_frames(recordings_path, rec->vrc_file);
			...
			if(session->aframes == NULL && session->vframes == NULL) {
				error_code = JANUS_RTMP_ERROR_INVALID_RECORDING;
...
playdone:
		...
		if(result != NULL)
			json_object_set_new(event, "result", result);
		if(!sdp) {
			int ret = gateway->push_event(msg->handle, &janus_rtmp_plugin, msg->transaction, event, NULL);
			JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(event);
		} else {
			...
			int res = gateway->push_event(msg->handle, &janus_rtmp_plugin, msg->transaction, event, jsep);
...
void janus_rtmp_update_recordings_list(void) {
	...
	while((recent = readdir(dir))) {
		...
		janus_rtmp_recording *rec = g_hash_table_lookup(recordings, &id);
		if(rec != NULL) {
		...
		if(audio && audio->value) {
			...
			rec->acodec = janus_audiocodec_from_name(janus_rtmp_parse_codec(recordings_path, rec->arc_file));
		}
		if(video && video->value) {
			...
			if(ext != NULL)
				*ext = '\0';
			/* Check which codec is in this recording */
			rec->vcodec = janus_videocodec_from_name(janus_rtmp_parse_codec(recordings_path, rec->vrc_file));
		}
		rec->audio_pt = AUDIO_PT;
		if(rec->acodec != JANUS_AUDIOCODEC_NONE) {
			/* Some audio codecs have a fixed payload type that we can't mess with */
		...
		rec->video_pt = VIDEO_PT;
		rec->viewers = NULL;
		if(janus_rtmp_generate_offer(rec) < 0) {
			JANUS_LOG(LOG_WARN, "Could not generate offer for recording %"SCNu64"...\n", rec->id);
		}
		g_atomic_int_set(&rec->destroyed, 0);
	...
	if(old_recordings != NULL) {
		while(old_recordings != NULL) {
			...
			janus_rtmp_recording *old_rec = g_hash_table_lookup(recordings, &id);
			if(old_rec != NULL) {
				/* Remove it */
...
janus_rtmp_frame_packet *janus_rtmp_get_frames(const char *dir, const char *filename) {
	...
	offset = 0;
	janus_rtmp_frame_packet *list = NULL, *last = NULL;
	while(offset < fsize) {
		/* Read frame header */
		fseek(file, offset, SEEK_SET);
		bytes = fread(prebuffer, sizeof(char), 8, file);
		...
		janus_rtmp_frame_packet *p = g_malloc(sizeof(janus_rtmp_frame_packet));
		...
		if(list == NULL) {
			...
			int added = 0;
			janus_rtmp_frame_packet *tmp = last;
			while(tmp) {
				if(tmp->ts < p->ts) {
	...
	janus_rtmp_frame_packet *tmp = list;
	count = 0;
	while(tmp) {
		count++;
...
static void *janus_rtmp_playout_thread(void *data) {
	janus_rtmp_session *session = (janus_rtmp_session *)data;
	if(!session) {
	...
	janus_refcount_increase(&session->recording->ref);
	janus_rtmp_recording *rec = session->recording;
	if(session->recorder) {
		janus_refcount_decrease(&rec->ref);
		janus_refcount_decrease(&session->ref);
	...
	janus_rtmp_frame_packet *audio = session->aframes, *video = session->vframes;
	...
	janus_rtmp_frame_packet *tmp = NULL;
