#ifndef _JANUS_RTMP_H
#define _JANUS_RTMP_H
...
#define JANUS_RTMP_BUFFER_MAX  2 * 1024 * 1024
#define htonll(x) ((1==htonl(1)) ? (x) : ((gint64)htonl((x) & 0xFFFFFFFF) << 32) | htonl((x) >> 32))
#define ntohll(x) ((1==ntohl(1)) ? (x) : ((gint64)ntohl((x) & 0xFFFFFFFF) << 32) | ntohl((x) >> 32))

typedef struct janus_rtp_jb			janus_rtp_jb;
typedef struct janus_rtmp_pub 		janus_rtmp_pub;
typedef struct janus_rtmp_el 		janus_rtmp_el;
...
typedef struct janus_rtmp_pub {
	...
	GSource 						*jb_src;
	GSource 						*pub_src;
	janus_rtmp_el 					*jb_loop;
	janus_rtmp_el 					*pub_loop;
	...
	janus_mutex 					mutex;
	janus_mutex 					mutex_rtmp;
	volatile gint 					destroyed;
	janus_refcount 					ref;
} janus_rtmp_pub;


typedef struct janus_rtmp_el {
	int 							id;
	char 							*name;
	GThread 						*thread;
	GMainLoop 						*mainloop;
...
janus_rtmp_pub *janus_rtmp_pub_create(const char *url, const char *acodec, const char *vcodec);
int janus_rtmp_pub_save_frame(janus_rtmp_pub *pub, char *buffer, uint length, gboolean video, int slot);
int janus_rtmp_pub_close(janus_rtmp_pub *pub);
void janus_rtmp_pub_destroy(janus_rtmp_pub *pub);
