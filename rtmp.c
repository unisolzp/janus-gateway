...
static void janus_rtmp_event_loop_init(janus_rtmp_pub *pub, janus_rtmp_el **el, int id, char* name);
static void *janus_rtmp_event_loop_thread(void *data);
static gboolean janus_rtmp_send_handle(gpointer user_data);
static gboolean janus_rtp_jb_handle(gpointer user_data);

static int janus_rtmp_ffmpeg_init(janus_rtmp_pub *pub);
static int janus_rtmp_ffmpeg_free(janus_rtmp_pub *pub);
static void janus_rtp_jb_free(janus_rtmp_pub *pub);
static janus_frame_packet *janus_packet_alloc(int data_len);
static void janus_packet_free(janus_frame_packet *pkt);
static void janus_rtmp_pub_free(const janus_refcount *pub_ref);

static int janus_rtmp_rtp_header_extension_parse_audio_level(char *buf,int len, int id, int *level);
static int janus_rtmp_rtp_header_extension_parse_video_orientation(char *buf, int len, int id, int *rotation);
static void janus_rtmp_h264_parse_sps(char *buffer, int *width, int *height);
static void janus_rtmp_rtp_unpack(janus_rtp_jb *jb, janus_frame_packet *packet, gboolean video);
static int janus_rtmp_packet_insert(janus_rtmp_pub *pub, janus_frame_packet *p);

static janus_adecoder_opus *janus_rtmp_opus_decoder_create(uint32_t samplerate, int channels, gboolean fec);
static void janus_rtmp_opus_decoder_destory(janus_adecoder_opus *dc);
static void janus_rtmp_opus_decoder_decode(janus_adecoder_opus *dc, char *buf, int len);
static janus_aencoder_fdkaac *janus_rtmp_fdkaac_encoder_create(int sample_rate, int channels, int bitrate);
static void janus_rtmp_fdkaac_encoder_destory(janus_aencoder_fdkaac *ec);
static void janus_rtmp_fdkaac_encoder_encode(janus_aencoder_fdkaac *ec, char *data, int len, uint32_t pts);
static void janus_rtmp_fdkaac_encoder_encode_inernal(janus_aencoder_fdkaac *ec, char *data, int len, uint32_t pts);


janus_rtmp_pub *
janus_rtmp_pub_create(const char *url, const char *acodec, const char *vcodec)
{
	if(url == NULL) {
		JANUS_LOG(LOG_ERR, "Missing rtmp url information\n");
		return NULL;
	}
    ...
    /* Create the rtmp pub */
	janus_rtmp_pub *pub = g_malloc0(sizeof(janus_rtmp_pub));
    ...
    if(pub->acodec) {
        ...
        pub->audio_jb->adecoder = janus_rtmp_opus_decoder_create(48000, 2, TRUE);
        pub->audio_jb->adecoder->jb = pub->audio_jb;
        pub->audio_jb->aencoder = janus_rtmp_fdkaac_encoder_create(48000, 2, 128);
    ...
    if(pub->vcodec) {
        ...
        pub->video_jb->buflen = JANUS_RTMP_BUFFER_MAX;
    ...
    janus_rtmp_event_loop_init(pub, &pub->jb_loop, id, tname);
    if(pub->jb_loop){
    ...
    janus_rtmp_event_loop_init(pub, &pub->pub_loop, id, tname);
    if(pub->pub_loop){
        pub->pub_src = g_timeout_source_new(50);
        g_source_set_priority(pub->pub_src, G_PRIORITY_DEFAULT);
        g_source_set_callback(pub->pub_src, janus_rtmp_send_handle, pub, NULL);
        g_source_attach(pub->pub_src, pub->pub_loop->mainctx);
    }

    janus_mutex_init(&pub->mutex);
    janus_mutex_init(&pub->mutex_rtmp);
    /* Done */
	g_atomic_int_set(&pub->destroyed, 0);
	janus_refcount_init(&pub->ref, janus_rtmp_pub_free);
    return pub;
}


void
janus_rtmp_event_loop_init(janus_rtmp_pub *pub, janus_rtmp_el **el, int id, char* name)
{
    GError *error       = NULL;
    janus_rtmp_el *loop = NULL;

    loop = g_malloc0(sizeof(janus_rtmp_el));
    loop->id = id;
    ...
    loop->thread = g_thread_try_new(loop->name, 
                    &janus_rtmp_event_loop_thread,loop, &error);
    if(error != NULL) {
        g_free(loop->name);
	    loop->name = NULL;
        loop->pub = NULL;
...
int
janus_rtmp_pub_save_frame(janus_rtmp_pub *pub, char *buffer, uint length, gboolean video, int slot)
{
	if(!pub)
		return -1;
	if(!buffer || length < 1) {
    ...
    if(rtp->extension) {
        ...
        if(pub->audio_level_extmap_id > 0)
            janus_rtmp_rtp_header_extension_parse_audio_level(buffer, length, pub->audio_level_extmap_id, &audiolevel);
        if(pub->video_orient_extmap_id > 0) {
            janus_rtmp_rtp_header_extension_parse_video_orientation(buffer, length, pub->video_orient_extmap_id, &rotation);
    ...
    if(video)
        JANUS_LOG(LOG_VERB, "janus_rtmp_pub_save_frame video ts: %"SCNu64"\n", p->ts);
...
void *
janus_rtmp_event_loop_thread(void *data)
{
	janus_rtmp_el *loop = data;
    janus_rtmp_pub *pub = (janus_rtmp_pub *)loop->pub;
...
{
	janus_rtmp_pub *pub = (janus_rtmp_pub *)user_data;
    ...
    while(head){
        ...
        if(now - head->created > G_USEC_PER_SEC){
            ...
            if(jb->adecoder){
                janus_rtmp_opus_decoder_decode(jb->adecoder, head->data, head->len);
            }
        ...
        if(now - head->created > G_USEC_PER_SEC){
            ...
            if(!head->drop){
                if(pub->video_jb->ts != head->ts && pub->video_jb->frameLen) {
                    ...
                    janus_rtmp_packet_insert(pub, p);
                    pub->video_jb->frameLen = 0;
                    pub->video_jb->keyFrame = 0;
                }

                janus_rtmp_rtp_unpack(pub->video_jb, head, TRUE);
...
int
janus_rtmp_packet_insert(janus_rtmp_pub *pub, janus_frame_packet *p)
{
    janus_mutex_lock_nodebug(&pub->mutex_rtmp);
    if(pub->start_ts == 0 && pub->start_sys == 0) {
        pub->start_ts = p->ts;
        pub->start_sys = janus_get_real_time();
    } 
    JANUS_LOG(LOG_INFO, "janus_rtmp_packet_insert, ts:%"SCNu64", len:%d, size:%d\n",  p->ts, p->len, pub->size);
    if(pub->list == NULL) {
        /* First element becomes the list itself (and the last item), at least for now */
        pub->list = p;
        pub->last = p;
    ...
    /* list size add */
    pub->size++;
	janus_mutex_unlock_nodebug(&pub->mutex_rtmp);
}


void
janus_rtmp_rtp_unpack(janus_rtp_jb *jb, janus_frame_packet *packet, gboolean video)
{
    janus_rtmp_pub *pub = jb->pub;
    int len = 0;
    ...
    if((buffer[0] & 0x1F) == 7) {
        ...
        janus_rtmp_h264_parse_sps(buffer, &width, &height);
        ...
        while(tot > 0) {
            ...
            if(nal == 7) {
                int width = 0, height = 0;
                janus_rtmp_h264_parse_sps(buf, &width, &height);
                JANUS_LOG(LOG_VERB, "Parsing width/height: %dx%d\n", width, height);
                if(width > pub->max_width)
                    pub->max_width = width;
                if(height > pub->max_height)
    ...
    if(!pub->init_flag && pub->max_width && pub->max_height) {
        int rc = janus_rtmp_ffmpeg_init(pub);
        if(rc == 0)
            pub->init_flag = TRUE;
    }
...
gboolean
janus_rtmp_send_handle(gpointer user_data)
{
	janus_rtmp_pub *pub = (janus_rtmp_pub *)user_data;
    gint64 now = janus_get_real_time();
    janus_frame_packet *tmp = NULL, *head = NULL;
    ...
    while(head){
        ...
        if(now - head->created > G_USEC_PER_SEC) {
            ...
            JANUS_LOG(LOG_INFO, "janus_rtmp_send_handle, %s packet len:%d,"
...
int
janus_rtmp_ffmpeg_free(janus_rtmp_pub *pub)
{
    if(pub->fctx != NULL)
...
int
janus_rtmp_ffmpeg_init(janus_rtmp_pub *pub)
{
    /* Setup FFmpeg */
	av_register_all();
...
void
janus_rtp_jb_free(janus_rtmp_pub *pub)
{
    janus_mutex_lock_nodebug(&pub->mutex);
    janus_frame_packet *tmp = NULL, *head = NULL;
    /*audio */
...
static int
janus_rtmp_rtp_header_extension_find(char *buf, int len, int id,
...
int
janus_rtmp_rtp_header_extension_parse_audio_level(char *buf, int len, int id, int *level)
{
	uint8_t byte = 0;
	if(janus_rtmp_rtp_header_extension_find(buf, len, id, &byte, NULL, NULL) < 0)
		return -1;
...
int
janus_rtmp_rtp_header_extension_parse_video_orientation(char *buf, int len, int id, int *rotation)
{
	uint8_t byte = 0;
	if(janus_rtmp_rtp_header_extension_find(buf, len, id, &byte, NULL, NULL) < 0)
		return -1;
...
void
janus_rtmp_pub_free(const janus_refcount *pub_ref)
{
    janus_frame_packet *tmp = NULL, *head = NULL;
	janus_rtmp_pub *pub = janus_refcount_containerof(pub_ref, janus_rtmp_pub, ref);
    ...
    JANUS_LOG(LOG_WARN, "janus rtmp pusblish release, list:%d\n", pub->size);
	g_free(pub);
}


int
janus_rtmp_pub_close(janus_rtmp_pub *pub)
{
	if(!pub)
		return -1;
	janus_mutex_lock_nodebug(&pub->mutex);
    ...
    pub->closed = TRUE;
    JANUS_LOG(LOG_WARN, "janus rtmp pusblish event thread closed\n");
	return 0;
}


void
janus_rtmp_pub_destroy(janus_rtmp_pub *pub)
{
...
void
janus_rtmp_h264_parse_sps(char *buffer, int *width, int *height)
{
...
janus_adecoder_opus *
janus_rtmp_opus_decoder_create(uint32_t samplerate, int channels, gboolean fec)
{
    int error = 0;
...
void
janus_rtmp_opus_decoder_destory(janus_adecoder_opus *dc)
{
    if(dc){
...
void
janus_rtmp_opus_decoder_decode(janus_adecoder_opus *dc, char *buf, int len)
{
    janus_rtp_header *rtp = (janus_rtp_header *)buf;
    ...
    if(seq_number == dc->expected_seq) {
        ...
        if(dc->fec && gap < AUDIO_DEFAULT_PREBUFFERING) {
            ...
            for(i=1; i<=gap ; i++) {
                ...
                janus_rtmp_fdkaac_encoder_encode(dc->jb->aencoder, data, length * dc->channels * sizeof(opus_int16), timestamp_tmp/(dc->jb->tb/1000));
            }
        }
    ...
    janus_rtmp_fdkaac_encoder_encode(dc->jb->aencoder, data, length * dc->channels * sizeof(opus_int16), timestamp/(dc->jb->tb/1000));
}


janus_aencoder_fdkaac *
janus_rtmp_fdkaac_encoder_create(int sample_rate, int channels, int bitrate)
{
...
void
janus_rtmp_fdkaac_encoder_destory(janus_aencoder_fdkaac *ec)
{
    if (ec->aframe) {
...
void
janus_rtmp_fdkaac_encoder_encode_inernal(janus_aencoder_fdkaac *ec, char *data, int len, uint32_t pts)
{
    if(!data || !len)
        return;
    gint64 now = janus_get_real_time();
    ...
    if ((ret_enc >= 0) && got_pic) {
        ...
        p->video = FALSE;
        p->ts = enc_pkt_p->dts;
        janus_rtmp_packet_insert(ec->jb->pub, p);
    }
...
void
janus_rtmp_fdkaac_encoder_encode(janus_aencoder_fdkaac *ec, char *data, int len, uint32_t pts)
{
    if(!data || !len)
        return;
    ...
    if(ec->buflen + len < total){
        ...
        janus_rtmp_fdkaac_encoder_encode_inernal(ec, ec->buffer, total, pts);
        ec->buflen = 0;
